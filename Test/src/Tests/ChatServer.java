...
import ru.geekbrains.lesson7.auth.JdbcAuthService;
...
public class ChatServer {
    ...
    private AuthService authService = new JdbcAuthService ("users.db");
    ...
    public void start(int port) throws Exception {
        try (ServerSocket serverSocket = new ServerSocket (port)) {
            ...
            log.debug ("Debug Message Logged !!!" + "Server started!");
            while (true) {
                ...
                log.info ("Info Message Logged !!!" + "New client connected!");
                ...
                try {
                    ...
                    if (matcher.matches ()) {
                        ...
                        if (authUser != null) {
                            ...
                            log.info ("Info Message Logged !!!" + "Authorization for user %s successful%n" + username);
                            updateUsersListForAllClient ();
                        } else {

                            //  System.out.printf("Authorization for user %s failed%n", username);
                            log.info ("Info Message Logged !!!" + "Authorization for user %s failed%n" + username);
                        ...
                        log.info ("Info Message Logged !!!" + "Incorrect authorization message %s%n" + authMessage);
        ...
        clientHandlerMap.forEach ((user, handler) -> {
            if (user.login.equals (userTo)) {
                try {
                    ...
                    log.info ("Info Message Logged !!!" + "User %s not found. Message from %s is lost.%n" + userTo + userFrom);
                }
            }
        });
    }
